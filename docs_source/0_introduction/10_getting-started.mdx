...
_&ldquo;"No, officer 👮‍♀️, I don't drv(thunk)!" &rdquo; &mdash; Erik Rasmussen_
...
But! Using `drv` brings in a few new possibilities:
...
First of all, we can use techniques like debounce to run our subscriptions less frequently. 
But more importantly, we can hold of the reactive system to wait a bit until we've done all our updates,
so that changes will be broadcasted as one atomic update. 
